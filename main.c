...
/* =========================================================================
   CALLBACK DE CARACTERES
   glfwSetCharCallback pasa por el sistema de texto del SO — funciona
   incluso cuando glfwGetKey falla por Wayland/IBus/layout de teclado.
   Maneja todas las teclas imprimibles: seleccion, pausa, reset, reflexion.
   ========================================================================= */
...
/* =========================================================================
   CALLBACK DE TECLAS ESPECIALES
   Para ESC, ESPACIO y TAB que NO son caracteres imprimibles.
   ========================================================================= */
...
/* =========================================================================
   ...
   Flechas y transformaciones de edificios responden suavemente.
   ========================================================================= */
static void teclas_hold(GLFWwindow *win)
{
#define P(k) (glfwGetKey(win, (k)) == GLFW_PRESS)
...
static void actualizar(GLFWwindow *win)
{
    ...
    if (ESTADO.vehiculo_tx > 1050.0f) ESTADO.vehiculo_tx = -300.0f;

    ESTADO.angulo_rueda -= 0.07f;
    if (ESTADO.angulo_rueda < -6.2832f) ESTADO.angulo_rueda += 6.2832f;

    ESTADO.bus_tx -= 1.5f;
    if (ESTADO.bus_tx < -260.0f) ESTADO.bus_tx = 960.0f;

    ESTADO.bus_angulo += 0.07f;
    if (ESTADO.bus_angulo > 6.2832f) ESTADO.bus_angulo -= 6.2832f;

    ESTADO.nube_tx -= 0.5f;
    if (ESTADO.nube_tx < -130.0f) ESTADO.nube_tx = 960.0f;
...
int main(int argc, char **argv)
{
    ...
    /* Sticky keys: garantiza que pulsaciones rapidas no se pierdan */
    glfwSetInputMode(ventana, GLFW_STICKY_KEYS, GLFW_TRUE);

    glfwSetFramebufferSizeCallback(ventana, cb_tamano);
    glfwSetCharCallback(ventana, cb_char);   /* caracteres: 1-4, 0, r/R */
    glfwSetKeyCallback(ventana, cb_key);     /* especiales: ESC, SPACE, TAB */
